...
from cs7313_mdp.mdp import FiniteStateMDP, MDPState
import itertools
import numpy as np
...
def _clip(p, max_x, max_y): #####might need to adjust up 1
...
class DiscreteGridWorldMDP(FiniteStateMDP):
    def __init__(self, w, h, pit_cost = -10, goal_cost = 10, move_cost=-0.1):
        self._w = w
        self._h = h
    ...
    @property
    def states(self):
        poss_combos =  itertools.product(
            range(self.width), range(self.height)#, (True, False), (True, False)
        )
        states = [GridState(i[0], i[1]) for i in poss_combos]
        #print('poss states', states, type(states), type(states[0]), type(states[0].pos), states[0].pos)
        return states
    ...
    def initial_state(self):
        return WumpusState(0, 0, False, False, self.width, self.height) #bruh?
